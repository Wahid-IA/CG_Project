...
public class GoblinBossHealthBarUI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (goblinBoss.isAwakened && !isBarActive && !goblinBoss.isDead)
        {
            isBarActive = true;
        ...
        if (isBarActive)
        {
            if (bossBarFill != null)
            {
                float healthPercentage = Mathf.Clamp01(goblinBoss.currentHealth / goblinBoss.maxHealth);
                bossBarFill.fillAmount = Mathf.Lerp(bossBarFill.fillAmount, healthPercentage, Time.deltaTime * 10f);
            }

            if (bossStaggerBarFill != null)
            {
                float staggerPercentage = Mathf.Clamp01(goblinBoss.currentStagger / goblinBoss.maxStagger);
            ...
            if (goblinBoss.currentHealth <= 0 || goblinBoss.isDead)
            {
                healthBarContainer.SetActive(false);
            }
        }
